...
class Conv(nn.Module):
    ...
    def forward(self, x):
        _x = self.act(self.bn(self.conv(x)))
        if self.dropout is not None:
            _x = self.dropout(_x)
        return _x

    def forward_fuse(self, x):
        _x = self.act(self.conv(x))
        if self.dropout is not None:
            _x = self.dropout(_x)
        return _x
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, dropout=0.0):  # ch_in, ch_out, number, shortcut, groups, expansion, dropout
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
